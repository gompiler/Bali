  ...
  where
    ...
    extractInfo info = ConstantPool <$> zipWithM extractInfo' [0 ..] info
      where
        ...
        extractInfo' i cpi         = throwInfo i cpi
    throwInfo :: Int -> ConvInfo -> CpConv a
    throwInfo i cpi = throwError $ BadInfo i $ Just cpi
    get :: (ConstantPoolInfo -> Maybe a) -> GetInfo a
    get f info index =
      let i = fromIntegral index
          cpi = info !! i
          throw = throwInfo i cpi
    ...
    convNameAndType info (Left cpi) =
      ...
      where
        ...
        convRef classIndex nameAndTypeIndex = do
          className <- getInfo info classIndex
